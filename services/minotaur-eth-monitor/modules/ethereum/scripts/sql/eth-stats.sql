SELECT
  (SELECT COUNT(*) FROM transactions) AS "tx count",
  (SELECT COUNT(*) FROM addresses) AS "address count",
  (SELECT COUNT(*) FROM blocks) AS "block count",
  (SELECT "blockIndex" FROM last_blocks WHERE currency = 2) AS "last block",
  ...
  (SELECT COUNT(*) FROM internal_transactions) AS "internal tx count",
