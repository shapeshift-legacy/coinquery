...
export function getRandomBigNumberInclusive(min: number, max: number): BigNumber {
  ...
  return new BigNumber(Math.floor(Math.random() * (maxFloor - minCeil + 1)) + minCeil)
}
...
export function getRandomDateOfBirth(
  year = getRandomIntInclusive(1920, 1998),
  month = getRandomIntInclusive(1, 9),
  day = getRandomIntInclusive(10, 28)
): string {
