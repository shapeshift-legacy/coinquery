...
CREATE OR REPLACE FUNCTION <%=schema%>.block_insert (
    ...
    BEGIN
        ...
        SELECT height, block_hash into var_last_block_height, var_last_block_hash
            FROM block
        ...
        -- Set next_block_hash of previous block if it is empty and in block does not equal last block
        IF var_last_block_hash != in_block_hash THEN
            UPDATE block
